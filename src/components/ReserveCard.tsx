...
const ReserveCard = ({ pg }: ReserveCardProps) => {
  ...
  const totalRent = nights * dailyRate;
  const serviceFee = Math.round(totalRent * 0.1);
  const totalAmount = totalRent + serviceFee;
  ...
  return (
    <div className="bg-card rounded-xl p-5 shadow-card border sticky top-24">
      {/* Price display */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <span className="text-2xl font-bold">₹{dailyRate.toLocaleString()}</span>
      ...
      {/* Date selection */}
      <div className="grid grid-cols-2 gap-0 border rounded-t-xl overflow-hidden">
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              className={cn(
                "h-auto py-3 px-4 justify-start text-left font-normal rounded-none border-r hover:bg-accent",
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              className={cn(
                "h-auto py-3 px-4 justify-start text-left font-normal rounded-none hover:bg-accent",
                !checkOutDate && "text-muted-foreground"
              )}
            >
              <div className="flex flex-col items-start">
                <span className="text-[10px] font-semibold uppercase tracking-wide">Checkout</span>
                <span className="text-sm">
      ...
      {/* Guest selection */}
      <div className="border border-t-0 rounded-b-xl mb-4">
        <Select value={guests} onValueChange={setGuests}>
          <SelectTrigger className="h-auto py-3 px-4 border-0 focus:ring-0 rounded-t-none">
        ...
        <div className="space-y-3 pt-4 border-t">
          <div className="flex justify-between text-sm">
            <span className="underline decoration-dotted">
              ₹{dailyRate.toLocaleString()} × {nights} nights
            </span>
